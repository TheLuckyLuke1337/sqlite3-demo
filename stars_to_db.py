...
BODY_INSERT = """
INSERT INTO CelestialBody (id, main_name, flux_V, temperature_K, body_class, radius_km, system_id)
VALUES (:oid, :main_name, :flux_v, :temperature, :sp_type, :radius_km, :system_id);
"""
...
def stars_from_simbad(radius=15.33):
    ...
    with sql.connect("database.sqlite3") as conn:
        ...
        for values in stars.iterrows():
            i += 1
            star = {key: value for key, value in zip(stars.colnames, values)}
            ...
            name = {"main_id": str(star["main_id"]), "oid": masked_to_int(star["oid"])}
